...
export default function SpecificCall() {
  ...
  function handleReport(e) {
    setCallDetails((prev) => ({ ...prev, report: e.target.value }));
  }
  ...
  }, [callDetails, meterIsValid]);

  useEffect(() => {
    if (currentCall?.lastMeterReading < callDetails?.meterReading) {
      setMeterIsValid(true);
    } else {
      setMeterIsValid(false);
    }
  }, [callDetails?.meterReading, currentCall?.lastMeterReading]);
