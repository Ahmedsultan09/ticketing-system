  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        input={<OutlinedInput id="select-multiple-chip" label="Select Operator/s" />}
        renderValue={(selected) => (
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
            {selected.map((operator) => (
              <Chip key={operator.id} label={operator.name} />
            ))}
          </Box>
        )}
        MenuProps={MenuProps}
      >
        {data.map((ele) => (
          <MenuItem
            key={ele.id}
            value={ele} // Pass the entire operator object as the value
