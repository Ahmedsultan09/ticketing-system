...
export default function DoneRvs() {
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
        ...
        <div>
          ...
          {selectedClient?.timeline?.length > 0 && (
            <Select
              ...
              value={new Date(
                selectedClient?.timeline[0].startDate
              ).getFullYear()}
            >
              ...
              <SelectContent>
                {selectedClient?.timeline?.map((time) => (
                  <SelectItem
                    key={time}
                    value={new Date(time.startDate).getFullYear()}
                  >
                    {new Date(time.startDate).getFullYear()}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      {selectedBranch && (
        <Tabs defaultValue={mockData.quarters[0].id.toString()}>
          <TabsList>
            {mockData.quarters.map((quarter) => (
              <TabsTrigger key={quarter.id} value={quarter.id.toString()}>
                {quarter.name}
              </TabsTrigger>
            ))}
          </TabsList>
          {mockData.quarters.map((quarter) => (
            <TabsContent key={quarter.id} value={quarter.id.toString()}>
              <Card>
                <CardHeader>
                  <CardTitle>
                    {quarter.name} Visits for {selectedBranch.name}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p>
                    <strong>Period:</strong> {quarter.startDate} to{" "}
                    {quarter.endDate}
                  </p>
                  {getVisitForBranch(selectedBranch.id, quarter.id) ? (
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {selectedBranch.machines.map(renderMachineCard)}
                      </div>
                    </div>
                  ) : (
