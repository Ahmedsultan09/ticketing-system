...
function ChildModal({
  ...
  useEffect(() => {
    if (isExistingMachine) {
      setDisabledBtn(
        !(matchedMachine && Object.keys(matchedMachine).length > 0) ||
          sameSerial
      );
    } else if (!isExistingMachine) {
      setDisabledBtn(!isFormValid || sameSerial);
    }
  }, [isFormValid, matchedMachine, isExistingMachine, sameSerial]);
...
export default function AddMachineModal({
  ...
  function handleAddMachine() {
    axiosInstance
      ...
      .then((res) => {
        ...
        handleClose();
        axiosInstance.get(`/regularVisits/${currentClient.id}`).then((res) => {
          const regularVisits = res.data;

          const updatedPeriods = regularVisits.periods.map((period) => {
            return { ...period, governorates: clientData.governorates };
          });
          const updatedYears = regularVisits.years.map((year) => {
            return { ...year, updatedPeriods };
          });

          return axiosInstance.put(`/regularVisits/${currentClient.id}`, {
            ...currentClient,
            years: updatedYears,
          });
        });
  ...
  }, [machineData, sameSerial]);

  useEffect(() => {
    // handling duplicate machine from searched existing machines

    if (machines.length > 0 && Object.keys(matchedMachine).length > 0) {
      const duplicateExistingMachine = machines.find((machine) => {
        return (
          machine.serialNumber.toString().toUpperCase() ===
          matchedMachine.serialNumber.toString().toUpperCase()
        );
      });

      if (
        matchedMachine.serialNumber.toString().toUpperCase() ===
        duplicateExistingMachine?.serialNumber.toString().toUpperCase()
      ) {
        setSameSerial(true);
      } else {
        setSameSerial(false);
  ...
  }, [machines, matchedMachine.serialNumber, matchedMachine]);

  useEffect(() => {
    // handling duplicate machine from searched non-existed machines

    if (machines.length > 0) {
      const duplicateNewMachine = machines.find((machine) => {
        return (
          machine.serialNumber.toString().toUpperCase() ===
          machineData.serialNumber.toString().toUpperCase()
        );
      });

      if (duplicateNewMachine && Object.keys(duplicateNewMachine).length > 0) {
        setSameSerial(true);
      } else {
        setSameSerial(false);
      }
    }
  }, [machines, machineData.serialNumber]);
        ...
        >
                ...
                {serialNumber !== "" &&
                  Object.keys(matchedMachine).length > 0 && (
                    <div className="w-full">
                      ...
                      />
                      {sameSerial && (
                        <p className="w-full flex justify-center items-center font-bold opacity-50 text-red-700">
                          This machine is already added in branch machines list{" "}
                        </p>
                      )}
                    </div>
                  )}
          ...
          {!isExistingMachine && (
            <div className="flex flex-row flex-wrap gap-3 items-start justify-between mt-2">
              ...
              {sameSerial && (
                <p className="w-full flex justify-center mb-4 items-center font-bold opacity-50 text-red-700">
                  This machine is already added in branch machines list{" "}
                </p>
              )}
            </div>
