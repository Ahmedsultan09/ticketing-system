...
function ChildModal({
  ...
  useEffect(() => {
    if (isExistingMachine) {
      setDisabledBtn(!Object.keys(matchedMachine).length > 0 || sameSerial);
...
export default function AddMachineModal({
  ...
  useEffect(() => {
    ...
    const duplicateExistingMachine = machines.find((machine) => {
      return machine.serialNumber === matchedMachine.serialNumber;
    });

    if (Object.keys(matchedMachine.serialNumber).length > 0) {
      if (
  ...
  }, [machines, matchedMachine.serialNumber]);
  ...
  useEffect(() => {
    ...
    if (Object.keys(duplicateNewMachine).length > 0) {
      setSameSerial(true);
    } else {
      setSameSerial(false);
    }
