...
function ClientInfo({ clients, isRvAllowed, timeline }) {
  ...
  const [rvAllowed, setRvAllowed] = useState(false);
  ...
  function handleChangeRvAllowed(e) {
    setRvAllowed(e.target.value);
  }

  useEffect(() => {
    console.log(rvAllowed);
  }, [rvAllowed]);
  ...
  return (
    <div className="w-full h-full p-3 border mt-2 border-gray-300 rounded-2xl relative">
      ...
      {!isEdit && isRvAllowed && (
        <div className="w-full h-1/5 border-b rounded-xl border-gray-400 p-3 font-normal flex flex-col lg:flex-row items-center justify-start relative mt-2 ">
          <div className="w-auto h-full opacity-60 font-normal mr-1">
            Timeline:{" "}
            {timeline.map((ele) => {
              return (
                <span
                  className="font-bold opacity-100 border border-gray-400 rounded-lg p-2 mr-2"
                  key={ele.startDate}
                >
          ...
          <Box sx={{ minWidth: 300 }}>
            <FormControl fullWidth size="small">
              <InputLabel id="demo-simple-select-label">
                Is Regular Visits Allowed?
              </InputLabel>
              <Select
                labelId="demo-simple-select-label"
                id="demo-simple-select"
                value={rvAllowed}
                label="Is Regular Visits Allowed?"
