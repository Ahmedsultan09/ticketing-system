...
function SpecificArea() {
  ...
  useEffect(() => {
    if (governorates.length > 0) {
      // Check if governorates is an array and has elements
      const currentGov = governorates
        ?.find((gov) => gov.id === currentGovID)
        ?.areas.find((area) => area.id === currentAreaID); // Use optional chaining to avoid errors if governorate or area is not found
      setCurrentGov(currentGov || {}); // Fallback to an empty object if no match is found
    }

    console.log(currentGov);
  }, [currentGov, currentAreaID, currentGovID, governorates]);
